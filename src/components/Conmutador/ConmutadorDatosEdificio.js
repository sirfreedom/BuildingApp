...
export class ConmutadorDatosEdificio extends React.Component {
    constructor(props) {
        ...
        this.cerrarLiquidacion = this.cerrarLiquidacion.bind(this);
    ...
    onContextMenuItems(e) {
        if (e.row && e.row.rowType === "data") {
            e.items = [{ text: "Ver departamentos", onItemClick: () => this.goToDepartamentos(e.row) },
                { text: "Cerrar liquidación", onItemClick: () => this.cerrarLiquidacion(e.row) }];
    ...
    goToDepartamentos(row) {
        window.history.pushState({ edificioId: row.data.edificioId }, "", "/departamento");
    }

    cerrarLiquidacion(row) {
        LiquidacionService.cerrarLiquidacion({ edificioId: row.data.edificioId }, () => {
            Notifier.mostrarNotificacion("operación exitosa", "success", 3000);
            this.refrescar();
        }, null);
    }
